...
/* =========================================
   GLOBAL STATE SELECTION ENFORCEMENT
   ========================================= */
function bindStateGating() {
  // Find the toast element by its class
  const toast = document.querySelector('.state-toast');
  if (toast) toast.style.display = 'none'; // Hide by default
  ...
  function showToast() {
    ...
    // Clear any existing timer so it doesn't hide too early if they click twice
  ...
  function isStateSelected() {
    // Look for the desktop or mobile state picker
    const pickers = document.querySelectorAll('select[name="state-picker"], [data-aed-state-picker="1"]');
  ...
  // Intercept clicks globally before they trigger page loads or Webflow interactions
  document.addEventListener('click', function(e) {
    // 1. Find what the user actually clicked on (a link, a button, etc.)
    const link = e.target.closest('a, button');
    
    // 2. Check if the click happened inside the Sample Programs wrapper
    const requireStateWrapper = e.target.closest('[data-aed-require-state="1"]');

    let needsState = false;

    if (requireStateWrapper && link) {
      // They clicked a button inside the Sample Programs section
      needsState = true;
    } else if (link) {
      // Check if they clicked a "Pricing" or "Create Program" link anywhere
      const href = (link.getAttribute('href') || '').toLowerCase();
      const text = (link.textContent || '').toLowerCase();

      if (href.includes('pricing') || href.includes('create-program') || text.includes('create program')) {
        needsState = true;
      }
    }

    // 3. If the item needs a state, check if one is selected!
    if (needsState) {
      if (!isStateSelected()) {
        e.preventDefault();   // Stop the link from opening
        e.stopPropagation();  // Stop Webflow from doing anything else
        
        showToast();          // Show your custom toast message
        
        // Give the state picker a quick red highlight so they know where to look
        const pickers = document.querySelectorAll('select[name="state-picker"]');
        pickers.forEach(picker => {
          picker.style.boxShadow = '0 0 0 2px #c62828';
          setTimeout(() => picker.style.boxShadow = '', 2000);
        });
      }
    }
  }, true); // The 'true' ensures this runs FIRST, before normal click events
}

// Run the gatekeeper!
setTimeout(bindStateGating, 500);
