...
function bindCheckboxSync() {
  console.log("✅ Smart Checkbox Sync v15 loaded!"); 
  ...
  // ─── Dynamic pill sync map (v15) ──────────────────────────────────────
  // Maps dynamic curriculum learning-area keys → the Webflow checkbox that
  // represents that LA.  For Y9-10, the checkbox must only be checked when
  // at least one dynamic pill is selected inside the corresponding
  // data-learning-area section.  This prevents the "LA toggled on, zero
  // subjects chosen" invalid state.
  var dynamicSyncMap = [
    { learningArea: 'the_arts',      cb: 'y10-arts-cb', container: 'y10-curriculum-container' },
    { learningArea: 'the_arts',      cb: 'y9-arts-cb',  container: 'y9-curriculum-container' },
    { learningArea: 'technologies',  cb: 'y10-tech-cb', container: 'y10-curriculum-container' },
    { learningArea: 'technologies',  cb: 'y9-tech-cb',  container: 'y9-curriculum-container' },
    { learningArea: 'hass',          cb: 'y10-hass-cb', container: 'y10-curriculum-container' },
    { learningArea: 'hass',          cb: 'y9-hass-cb',  container: 'y9-curriculum-container' },
    // NSW / VIC / other variant keys
    { learningArea: 'creative_arts',                   cb: 'y10-arts-cb', container: 'y10-curriculum-container' },
    { learningArea: 'creative_arts',                   cb: 'y9-arts-cb',  container: 'y9-curriculum-container' },
    { learningArea: 'technological_and_applied_studies',cb: 'y10-tech-cb', container: 'y10-curriculum-container' },
    { learningArea: 'technological_and_applied_studies',cb: 'y9-tech-cb',  container: 'y9-curriculum-container' },
    { learningArea: 'hsie',                            cb: 'y10-hass-cb', container: 'y10-curriculum-container' },
    { learningArea: 'hsie',                            cb: 'y9-hass-cb',  container: 'y9-curriculum-container' },
    { learningArea: 'humanities',                      cb: 'y10-hass-cb', container: 'y10-curriculum-container' },
    { learningArea: 'humanities',                      cb: 'y9-hass-cb',  container: 'y9-curriculum-container' }
  ];
  ...
  function updateCheckboxes() {
    ...
    // ── 2. Dynamic curriculum pill sync (v15) ──
    // For each entry in dynamicSyncMap, count .aed-dynamic-pill.is-selected
    // in the data-learning-area section inside the container.  If the
    // container is visible and has a rendered section, sync the checkbox.
    // We accumulate counts per checkbox ID because multiple learning-area
    // variants (e.g. "the_arts" + "creative_arts") map to the same cb.
    var dynamicCounts = {}; // cbId → total selected dynamic pills
    dynamicSyncMap.forEach(function(item) {
      var containerEl = document.getElementById(item.container);
      if (!containerEl || containerEl.offsetParent === null) return;

      var section = containerEl.querySelector('[data-learning-area="' + item.learningArea + '"]');
      if (!section) return;

      var count = section.querySelectorAll('.aed-dynamic-pill.is-selected').length;
      dynamicCounts[item.cb] = (dynamicCounts[item.cb] || 0) + count;
    });

    // Now sync checkboxes based on dynamic pill counts
    Object.keys(dynamicCounts).forEach(function(cbId) {
      var cbs = document.querySelectorAll('#' + cbId);
      var element = Array.from(cbs).find(function(el) { return el.offsetParent !== null; }) || cbs[0];
      if (!element) return;
      ...
      if (realInput.classList.contains('locked-checkbox') || wrapper.classList.contains('locked-checkbox')) return;

      var shouldBeChecked = (dynamicCounts[cbId] > 0);
  ...
  // v15: Also sync when dynamic curriculum pills change
